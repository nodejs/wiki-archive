...
The following is a list of some interesting api bits that aren't considered safe to use in a web setting but are built into node's V8 engine.

V8 implements all of "ECMA 5th edition":http://www.ecma-international.org/publications/standards/Ecma-262.htm
...
@var obj = { get a() { return "something" }, set a(value) { "do nothing" } }@ getter/setter syntax
@for (i in a) { ... }@ for...in iteration
