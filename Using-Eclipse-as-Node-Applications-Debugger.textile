...
Now you can examine stack frame, browse program variables - and do all the other useful stuff. If the same script was started with:
...
You probably noticed that Eclipse V8 debugger can not connect to the remote host. This feature request s already pending as low priority item. However it is very easy to use simple transparent TCP proxy script to enable your development machine to debug your application directly on the deployment or test server:
...
var encoding = "binary"
var localActive = false;  // flag to majke sure that only one connection
                          // to remote is active
...
<pre>
<code>
DevMachine:$ node tcpproxy.js 5858 www.example.com 5858 &
</code>
</pre>
...
<pre>
<code>
RemoteMachine:$ node --debug-brk MyApp.js
</code>
</pre>

Now you can start debugging session on local port 5858 as usual. Note that tcpproxy.js needs to be started only once for particular remote host and port combination. If verbose option is set for the above script, it can be used as a sniffer for "V8 Debugger protocol":http://code.google.com/p/chromedevtools/wiki/ChromeDevToolsProtocol as well.

Eclipse V8 debugger can also be used in conjunction with   gdb/ddd in order to debug native C/C++ code. For example, if particular invocation of frequently called native function is problematic, but all other ones are fine, you would first set the breakpoint in the script code, and once you hit this one, then you would set/enable the breakpoint in native function  as well.
