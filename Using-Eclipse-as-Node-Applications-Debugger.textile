...
You probably noticed that Eclipse V8 debugger can not connect to the remote host. Moreover, the V8 debugger server thread only accept connections at 127.0.0.1 (localhost), so debugging Node application on remote host requires usage of simple TCP transparent proxy Node script both on local machine running the Eclipse debugger and on remote target as well:
...
Now the  Eclipse debugging session will be routed via localhost:5858 to www.example.com:8888 and then to remote target localhost:7777.  If verbose option is set, the tcpproxy.js may act as a sniffer for "V8 Debugger protocol":http://code.google.com/p/chromedevtools/wiki/ChromeDevToolsProtocol as well.
