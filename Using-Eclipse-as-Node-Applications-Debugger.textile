First, there is new project "Nodeclipse":http://www.nodeclipse.org/ project, that lets to create, edit and debug Node.js application in fully integrated way. It uses the same "Eclipse debugger plugin for V8":http://code.google.com/p/chromedevtools/ (with some bugs fixed). The usage may be simplified, but the principle stays the same.
...
_Note:_ The most recent version of Eclipse V8 debugger plugin has an option to specify remote host, so above proxy script is not required any more. It may still be useful if debugger is to reach out through corporate firewall.
...
_Note:_ If node segfaults when you are trying to debug, recompile node with set GCC_VERSION=44 before running make during node installation ([[source|http://blog.quilitz.de/2010/08/debug-nodejs-applications-with-eclipse-fix-segmentation-fault/]])