...
* event: @'close'@ (called when file descriptor is closed and no more events will be emitted)
* event: @'error'@ (emitted with exception object - does not close stream)
* method: @pause()@
* method: @resume()@
* member: @readable@, boolean
* method: @forceClose()@ (close the underlying file descriptor)
...
* method: @close()@ (Writes EOF or a FIN packet on the transport once the existing data buffer is drained. Further calls to @write()@ after calling this method will throw an error.)
* event: @'drain'@
* event: @'close'@ (Emitted after close)
* event: @'error'@(emitted with exception object - does not close stream)
