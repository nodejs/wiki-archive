...
* "RDX":http://github.com/kreetitech/RDX - An object-relational mapper (ORM) for node. Backends: node-mysql-libmysqlclient.
...
* "nodemon":http://github.com/remy/nodemon -- Monitors all source files, restarts node on changes. Command-line usage, code-passive with ignore list.
...
* "oauth-server":http://github.com/selead/oauth-server -- OAuth server (1.0A)
...
* "Pulsar":https://github.com/michaelsbradleyjr/Pulsar -- An event emitter abstraction class; reuse your custom-emitter logic across JavaScript environments, transparently falling back to node's EventEmitter on the server-side.
...
* "Grillode":http://github.com/stephenmcd/grillode -- Chat server with various modes such as customer support queues, fixed rooms or Chatroulette random matchups. Written in CoffeeScript and uses express, socket.io and Coffeekup.
